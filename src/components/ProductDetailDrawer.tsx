...
export function ProductDetailDrawer({ product, open, onClose, onAddToOrder, siblings, onNavigate }: ProductDetailDrawerProps) {
  const [stage, setStage] = useState<'select' | 'secondary' | 'extras'>('select')
  ...
  // En la etapa de variantes secundarias, el botón queda bloqueado hasta elegir una opción.
  const variantBloqueada = stage === 'secondary'
    ? tieneVariantesSecundarias && !varianteSecundariaSeleccionada
    : false

  // Price calculation
  const precioBasePrimario = parseFloat(String(product?.precio ?? 0))
  ...
  const handleContinue = () => {
    ...
    if (stage === 'select' && tieneVariantesSecundarias) {
      setStage('secondary')
  ...
  const stage1Height = Math.min(
    stage1Base + (hasCarneSection ? LABEL_H + carneSectionRows * ROW_H : 0),
    Math.round(vh * 0.85)
  )
  ...
  // Etapa de variantes secundarias: mismo patrón que la etapa 1, con scroll interno a partir de 3 opciones.
        ...
        >
          {product ? (
            <>
              ...
              {/* Botón volver — en secundaria y extras, sobre la imagen (el diseño plano tiene el suyo propio) */}
              <AnimatePresence>
                {!sinImagen && stage !== 'select' && (
                  <motion.button
                    key="back"
                    type="button"
                    onClick={() => setStage(stage === 'extras' && tieneVariantesSecundarias ? 'secondary' : 'select')}
                      ...
                      {stage !== 'extras' ? (
                        ...
                        >
                          <div className="min-h-0 flex-1 space-y-6 overflow-y-auto overscroll-contain px-6 pb-4 pt-6">
                            {stage === 'select' && <p className="text-[15px] leading-relaxed text-foreground/70">
                              {product.descripcion || 'Sin descripción.'}
                            </p>}

                            {/* Carne selector: reemplaza las variantes de la tienda genérica */}
                            {stage === 'select' && hasCarneSection && (
                            ...
                            {/* Variantes secundarias — segunda opción obligatoria antes de continuar */}
                            {stage === 'secondary' && tieneVariantesSecundarias && (
                              <div className="space-y-1">
                                <p className="px-1 pb-1 text-[13px] font-medium text-muted-foreground">
                                  Elegí también una segunda opción
                                </p>
                                <div
                                  className="flex flex-col gap-0.5 overflow-y-auto overscroll-contain"
                                  style={{ maxHeight: MAX_VISIBLE_VARIANTS * ROW_H }}
                                >
                                  {product.variantesSecundarias!.map((v) => {
                                    const sel = varianteSecundariaSeleccionada?.id === v.id
                                    return (
                                      <button
                                        key={v.id}
                                        type="button"
                                        onClick={() => setVarianteSecundariaSeleccionada(v)}
                   ...
                   <div className="flex items-stretch gap-2">
                    ...
                    >
                      <AnimatePresence mode="wait">
                        ...
                        ) : stage === 'select' && (tieneVariantesSecundarias || needsStage2) ? (
                          <motion.span
                            key="continue"
                            initial={{ opacity: 0, y: 8 }}
                            animate={{ opacity: 1, y: 0 }}
